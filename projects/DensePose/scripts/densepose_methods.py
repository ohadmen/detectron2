...
class DensePoseMethods:
    ...
    def barycentric_coordinates_exists(self, P0, P1, P2, P):
        ...
        ok[np.dot(vCrossW, vCrossU) < 0]=False
        #
        uCrossW = np.cross(u, w)
        uCrossV = np.cross(u, v)
        #
        ok[np.dot(uCrossW, uCrossV) < 0]=False
        #
        denom = np.sqrt((uCrossV ** 2).sum())
        r = np.sqrt((vCrossW ** 2).sum(axis=1)) / denom
        t = np.sqrt((uCrossW ** 2).sum(axis=1)) / denom
        #
        ok[~( ((r <= 1) & (t <= 1) & (r + t <= 1)))]=False
        return ok
    ...
    def barycentric_coordinates(self, P0, P1, P2, P):
        ...
        r = np.linalg.norm(vCrossW,axis=1) / denom
        t = np.linalg.norm(uCrossW,axis=1) / denom
        #
        return (1 - (r + t), r, t)

    def IUV2FBC(self, I_point, U_point, V_point):
        P = np.c_[U_point, V_point, V_point*0]
        ...
        #
        fbc = np.ones((P.shape[0],4))*np.nan
        for i, [P0, P1, P2] in enumerate(zip(P_0, P_1, P_2)):
            ok = self.barycentric_coordinates_exists(P0, P1, P2, P)
            if np.any(ok):
                [bc1, bc2, bc3] = self.barycentric_coordinates(P0, P1, P2, P[ok])
                fbc[ok,0]=FaceIndicesNow[0][i]
                fbc[ok,1]=bc1
                fbc[ok, 2] = bc2
                fbc[ok, 3] = bc3
            if np.all(~np.isnan(fbc)):
                return fbc
    ...
    def FBC2PointOnSurface(self, fbc, Vertices):
        ##
        p = np.ones((fbc.shape[0],3))*np.nan
        ok =~np.isnan(fbc[:,0])
        Vert_indices = self.All_vertices[self.FacesDensePose[fbc[ok,0].astype(int)]] - 1
        ##
        p[ok] = Vertices[Vert_indices[:,0], :] * fbc[ok,1:2] + \
            Vertices[Vert_indices[:,1], :] * fbc[ok,2:3] + \
            Vertices[Vert_indices[:,2], :] * fbc[ok,3:4]
        ##
        return p
